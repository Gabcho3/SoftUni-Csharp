...
namespace CarDealer
{
    public class StartUp
    {
        ...
        public static string GetTotalSalesByCustomer(CarDealerContext context)
        {
            ...
            {
                if (customer.IsYoungerDriver)
                {
                    customer.SpentMoney *= 0.95m;
                }

                customer.SpentMoney = decimal.Parse($"{customer.SpentMoney:f2}");
            }

            customers = customers.OrderByDescending(c => c.SpentMoney)
                .ToArray();
